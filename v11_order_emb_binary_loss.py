from src.model_v11_order_emb_binary_loss import *
...
def init():
    ...
    args_parser.add_argument("--model_path", default='output/v11_order_emb_binary_loss', type=str)
    args_parser.add_argument("--patience", default=5, type=str)
    args_parser.add_argument("--alpha", default=0.0001, type=float)
...
if __name__ == '__main__':
    ...
    # Set parameters
    args = init()
    model_path = os.path.join(args.model_path, 'v11_' + args.bert_version, str(args.max_seq_length),
                              args.dataset + "_" + args.bert_pooling + "_" + args.sent_pooling
                              + "_" + str(args.error_margin) + "_" + str(args.threshold)) + "_" + str(args.alpha)

    if not os.path.exists(model_path):
        os.makedirs(model_path)

    model_params = [str(args.bert_version), "_ordemb_binary_loss"]
    ...
    fname_test = os.path.join(args.dataset_path, args.dataset + "_test.csv")
    ...
    # 3. Build model
    model = Bert_OrderEmb_Binary_LossFunc(args)
