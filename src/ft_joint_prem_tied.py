...
from src.models.joint_prem_tied import *
...
def init():
    ...
    args_parser.add_argument("--model_path", default='output/v16_joint_prem_tied', type=str)
...
if __name__ == '__main__':
    ...
    # Set parameters
    args = init()
    model_path = os.path.join(args.model_path, 'v16_' + args.bert_version, str(args.max_seq_length),
                              args.dataset + "_" + args.bert_pooling + "_" + args.sent_pooling + "_" +
                              str(args.error_margin) + "_" + str(args.threshold))
    ...
    for epoch in trange(args.epochs, desc="Epoch"):
        ...
        for step, batch in enumerate(tqdm(train_loader, desc="Iteration")):
            ...
            seq_in1 = tokenize_mask(sent_premises, args.max_seq_length, bert_tokenizer)
            seq_in2 = tokenize_mask(sent_hypos, args.max_seq_length, bert_tokenizer)
            ...
            train_f1score = f1_score(tb_labels, preds)
            train_acc = accuracy_score(tb_labels, preds)
            train_precision = precision_score(tb_labels, preds)
            train_recall = recall_score(tb_labels, preds)
        ...
        for step, batch in enumerate(tqdm(val_loader, desc="validation")):
            ...
            v_seq_in1 = tokenize_mask(v_sent_premises, args.max_seq_length, bert_tokenizer)
            v_seq_in2 = tokenize_mask(v_sent_hypos, args.max_seq_length, bert_tokenizer)
            ...
            avg_val_precision += precision_score(vb_labels, v_preds)
            avg_val_recall += recall_score(vb_labels, v_preds)
            avg_val_f1score += f1_score(vb_labels, v_preds)
