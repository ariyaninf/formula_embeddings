...
from src.models.joint_enc import *
...
def init():
    ...
    args_parser.add_argument("--model_path", default='output/joint_encoder', type=str)
...
if __name__ == '__main__':
    ...
    args = init()
    if args.OR_only:
        model_path = os.path.join(args.model_path, args.bert_version + "_Table1", str(args.batch_size),
                                  args.dataset + args.bert_pooling + args.emb_pooling + "_OR")
    else:
        model_path = os.path.join(args.model_path, args.bert_version + "_Table1", str(args.batch_size),
    ...
    model_params = [args.bert_version, str(args.lr), str(args.batch_size), str(args.weight_decay),
                    str(args.bert_pooling), str(args.emb_pooling), str(args.epochs), str(args.OR_only)]
    ...
    df_train = df_train.drop(columns=['sentence1', 'sat1', 'sentence2', 'sat2'])
    df_val = df_val.drop(columns=['sentence1', 'sat1', 'sentence2', 'sat2'])
