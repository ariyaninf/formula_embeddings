...
class Bert_OrderEmb_Clause_Loss(torch.nn.Module):
    def __init__(self, config):
        ...
        self.cl_lambda = config.clause_lambda
        self.register_buffer('model', None)

    def forward(self, seq_in1, seq_in2, seq_equ1, seq_equ2, labels, use_gpu):
        ...
        emb_in1, emb_clauses_in1 = self.get_sent_emb_premises(token_ids1, input_masks1)
        emb_in2 = self.get_sent_emb_hypos(token_ids2, input_masks2)
        emb_equ_clauses_in1 = self.get_sent_emb_equ_premises(token_ids_equ1, input_mask_equ1)
        emb_equ_in2 = self.get_sent_emb_equ_hypos(token_ids_equ2, input_mask_equ2)

        if torch.cuda.is_available():
            labels = torch.tensor(labels).to("cuda")

        loss, preds = self.en_loss(emb_in1, emb_in2, emb_clauses_in1, emb_equ_clauses_in1, emb_equ_in2, labels)
        return loss, preds
    ...
    def en_loss(self, emb_in1, emb_in2, emb_clauses_in1, emb_equ_clauses_in1, emb_equ_in2, en_labels):
        batch_size = emb_in1.shape[0]
        t_labels = en_labels
        device = emb_in1.device
        ...
        e_clauses = calc_clauses_loss(emb_clauses_in1, emb_equ_clauses_in1, emb_in2, emb_equ_in2)
        e = calc_penalty_pos(emb_in1, emb_in2)
        # print('e: ', )
        ...
        err_loss = err_pos + err_neg + (self.cl_lambda * e_clauses)
