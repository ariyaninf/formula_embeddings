...
from src.models.order_emb_0L import *
...
def init():
    ...
    args_parser.add_argument("--dataset", default='SENT_5-9vars_1-10clauses_100K_1-3sat_hy_25_ER', type=str)
    args_parser.add_argument("--dataset_path", default='dataset/train_set/Table_1/', type=str)
    args_parser.add_argument("--batch_size", default=16, type=int)
    args_parser.add_argument("--max_seq_length", default=32, type=int)
    args_parser.add_argument("--lm_version", default='bert_base_uncased', type=str)
    ...
    args_parser.add_argument("--emb_pooling", default='cls', type=str)  # cls, mean
    args_parser.add_argument("--sent_pooling", default='min', type=str)  # max, mean, min
    args_parser.add_argument("--error_margin", default=1, type=float)
    args_parser.add_argument("--threshold", default=0.5, type=float)
    args_parser.add_argument("--model_path", default='output/v6_order_embeddings', type=str)
    args_parser.add_argument("--patience", default=3, type=str)
...
if __name__ == '__main__':
    ...
    # Set parameters
    args = init()
    model_path = os.path.join(args.model_path, 'v6_' + args.lm_version, str(args.max_seq_length),
                              args.dataset + "_" + args.emb_pooling + "_" + args.sent_pooling + "_" +
                              str(args.error_margin) + "_" + str(args.threshold) + "_" + str(args.lr))

    if not os.path.exists(model_path):
        os.makedirs(model_path)

    model_params = [str(args.lm_version), "_order_emb"]

    model_params += [str(args.lr), str(args.batch_size), str(args.weight_decay), str(args.emb_pooling),
    ...
    model_tokenizer = AutoTokenizer.from_pretrained('cache/' + args.lm_version, local_files_only=True)
    ...
    df_train = df_train.drop(columns=['sentence1', 'sentence2', 'sat1', 'sat2'])
    df_val = df_val.drop(columns=['sentence1', 'sentence2', 'sat1', 'sat2'])
    ...
    for epoch in trange(args.epochs, desc="Epoch"):
        ...
        for step, batch in enumerate(tqdm(train_loader, desc="Iteration")):
            ...
            seq_in1 = tokenize_mask(sent_premises, args.max_seq_length, model_tokenizer, max_num_cl, 'premises')
            seq_in2 = tokenize_mask(sent_hypos, args.max_seq_length, model_tokenizer, max_num_cl, 'hypos')
        ...
        for step, batch in enumerate(tqdm(val_loader, desc="validation")):
            ...
            v_seq_in1 = tokenize_mask(v_sent_premises, args.max_seq_length, model_tokenizer, v_max_num_cl, 'premises')
            v_seq_in2 = tokenize_mask(v_sent_hypos, args.max_seq_length, model_tokenizer, v_max_num_cl, 'hypos')
