...
class Bert_OrderEmb_HD(torch.nn.Module):
    def __init__(self, config, bert_version='bert-base-uncased'):
        super(Bert_OrderEmb_HD, self).__init__()
        self.bert = BertModel.from_pretrained('cache/bert_base_uncased', output_hidden_states=True,
                                              local_files_only=True)
        self.in_dim = self.bert.config.hidden_size  # in_dim = 768
        self.bert_pooling = config.bert_pooling  # cls, mean
        self.sent_pooling = config.sent_pooling  # max
    ...
    def forward(self, token_ids1, input_masks1, token_ids2, input_masks2, labels):
        ...
        loss, preds = self.en_loss(emb_in1, emb_in2, labels, self.threshold)
        return loss, preds
    ...
    def get_sent_emb_premises(self, token_ids, input_masks):
        ...
        # print('pm_emb_all.shape: ', pm_emb_all.shape)
    ...
    def get_sent_emb(self, token_ids, input_masks):
        ...
        h1 = self.layer_out(pooled)

        del token_ids, last_hidden_states, pooled
        del input_masks
        return h1

    def en_loss(self, trans_in1, trans_in2, en_labels, threshold):
