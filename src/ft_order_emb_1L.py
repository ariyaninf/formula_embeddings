...
from src.models.order_emb_1L import *
...
def init():
    ...
    args_parser.add_argument("--out_dim", default=768, type=int)  # 768, 1000
    args_parser.add_argument("--error_margin", default=1, type=int)
    ...
    args_parser.add_argument("--patience", default=2, type=int)
    args_parser.add_argument("--OR_only", default=False, type=bool)
...
if __name__ == '__main__':
    ...
    args = init()
    if args.OR_only:
        model_path = os.path.join(args.model_path, 'v7_' + args.bert_version , str(args.max_seq_length),
                                  args.dataset + "_" + args.bert_pooling + "_" + args.sent_pooling + "_" +
                                  str(args.out_dim) + "_dim_" + str(args.error_margin) + "_" + str(args.threshold) +
                                  "_OR_only")
    else:
        model_path = os.path.join(args.model_path, 'v7_' + args.bert_version, str(args.max_seq_length),
                                  args.dataset + "_" + args.bert_pooling + "_" + args.sent_pooling + "_" +
    ...
    model_params = [str(args.bert_version), "_ordemb_HD"]
    ...
    if args.OR_only:
        train_premises = df_train.sat1.values
        train_hypos = df_train.sat2.values
        train_num_clauses = df_train.num_clauses.values

        val_premises = df_val.sat1.values
        val_hypos = df_val.sat2.values
        val_num_clauses = df_val.num_clauses.values
    else:
    ...
    # 3. Build model
    model = Bert_OrderEmb_HD(args, 'cache/' + args.bert_version)
    ...
    for epoch in trange(args.epochs, desc="Epoch"):
        ...
        for step, batch in enumerate(tqdm(train_loader, desc="Iteration")):
            ...
            # print('tb_indices: ', type(tb_indices))
            # print('tb_labels: ', batch[1])
