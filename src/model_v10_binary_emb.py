...
class BinaryEmb(torch.nn.Module):
    def __init__(self, config):
        super(BinaryEmb, self).__init__()
    ...
    def en_loss(self, trans_in1, trans_in2, en_labels):
        ...
        e = calc_biscore(trans_in1, trans_in2)
        # print('e: ', e)
...
class BinaryEmb_Boosting(torch.nn.Module):
    def __init__(self, config):
        super(BinaryEmb_Boosting, self).__init__()
        if 'albert' in config.bert_version:
            self.bert = AlbertModel.from_pretrained('cache/' + config.bert_version, output_hidden_states=True,
                                                    local_files_only=True)
        elif 'distilbert' in config.bert_version:
...
def calc_biscore(vec_p, vec_h):
    device = vec_p.device
    pen = torch.tensor([]).to(device)
    pen = torch.sub(torch.tensor([1]).to(device), torch.mul(vec_p, vec_h))
    pen = torch.mul(vec_p, pen)
