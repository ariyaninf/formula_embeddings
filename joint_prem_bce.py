...
class BiEncoder_BCE(torch.nn.Module):
    def __init__(self, config):
        super(BiEncoder_BCE, self).__init__()
        self.bert_prem = AutoModel.from_pretrained('cache/' + config.bert_version, output_hidden_states=True,
                                                   local_files_only=True)
        self.bert_hypo = AutoModel.from_pretrained('cache/' + config.bert_version, output_hidden_states=True,
                                                   local_files_only=True)
    ...
    def en_loss(self, trans_in1, trans_in2, en_labels):
        ...
        dot_products = torch.bmm(trans_in1.unsqueeze(dim=1), trans_in2.unsqueeze(dim=2)).squeeze()
        criterion = nn.BCEWithLogitsLoss()
        err_loss = criterion(dot_products, en_labels.float())

        outputs = torch.round(torch.sigmoid(dot_products))
